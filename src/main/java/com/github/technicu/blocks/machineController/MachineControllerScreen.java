...
@OnlyIn(Dist.CLIENT)
public class MachineControllerScreen extends ContainerScreen<MachineControllerContainer> {

    //<editor-fold>
    private static final ResourceLocation ALLOY_SMELTER = new ResourceLocation(Technicu.MOD_ID,"textures/gui/multiblock_alloy_smelter.png");
    private static final TranslationTextComponent TITLE = new TranslationTextComponent("container." + Technicu.MOD_ID + ".machine_controller");

    private MachineControllerContainer container;
    public MachineControllerScreen(MachineControllerContainer screenContainer, PlayerInventory playerInventory, ITextComponent title) {
    ...
    protected void renderTooltip(MatrixStack matrixStack, int mouseX, int mouseY) {
        ...
        if(mouseX >= getGuiLeft()+154 && mouseX < getGuiLeft()+154+12){
            if(mouseY >= getGuiTop()+18 && mouseY < getGuiTop()+18+50){
                ...
                this.renderTooltip(matrixStack, new StringTextComponent(current.get() + "/"+ MachineControllerTileEntity.MAX_ENERGY),mouseX,mouseY);
            }
        }
